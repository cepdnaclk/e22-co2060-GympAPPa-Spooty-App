...
function ReturnEquipment() {
    ...
    const handleSearch = async () => {
        ...
        setLoading(true);
        try {
            const response = await adminAPI.getPendingReturns(regNumber.trim());
    ...
    const handleReturn = async (requestId, equipmentName) => {
        ...
        setLoading(true);
        try {
            const response = await adminAPI.returnEquipment(requestId);
            setSuccessMsg(response.data.message);
            setItems(prev => prev.filter(i => i.request_id !== requestId));
        } catch (err) {
            setReturnError(err.response?.data?.message || 'Could not connect to server.');
        } finally {
            setLoading(false);
        }
    };
